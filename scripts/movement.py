...
class Movement :
	...
	def __init__(self):
		...
		self.movegroup_larm = moveit_commander.MoveGroupCommander("head") # Corresponds to the lower body joints
		self.movegroup_larm.set_goal_tolerance(self.tolerance)
		self.movegroup_larm.set_planning_time(self.planning_time)
	...
	def plan_6d_pose(self,pose):
		begin_time = rospy.get_time()
		self.movegroup_rarm.set_start_state_to_current_state()
		self.movegroup_rarm.set_pose_target(pose, end_effector_link ='r_gripper')
		plan = self.movegroup_rarm.plan()
		end_time = rospy.get_time()
		rospy.loginfo(f"Planning done in {end_time-begin_time}s")
		return plan
	...
	def plan_xyz_position(self, xyz):
		begin_time = rospy.get_time()
		self.movegroup_rarm.set_start_state_to_current_state()
		self.movegroup_rarm.set_position_target(xyz, end_effector_link ='r_gripper')
		plan = self.movegroup_rarm.plan()
		end_time = rospy.get_time()
		rospy.loginfo(f"Planning done in {end_time-begin_time}s")
		return plan
	...
	def plan_rpy_orientation(self, rpy):
		begin_time = rospy.get_time()
		self.movegroup_rarm.set_start_state_to_current_state()
		self.movegroup_rarm.set_rpy_target(rpy, end_effector_link ='r_gripper')
		plan = self.movegroup_rarm.plan()
		end_time = rospy.get_time()
		rospy.loginfo(f"Planning done in {end_time-begin_time}s")
		return plan
